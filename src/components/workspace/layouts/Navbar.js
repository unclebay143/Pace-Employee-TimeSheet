...
const Navbar = () => {
  ...
  useEffect(() => {
    const fetchCompanyProfile = () => {
      axios
        .get(FETCH_COMPANY_PROFILE_API + currentUserCompanyID, {
          headers: authHeader,
        })
    ...
    <>
      ...
      <header className="header">
        <nav className="navbar navbar-expand-lg px-4 py-2 bg-white shadow">
          ...
          <Link
            to={`/dashboard/profile/${staffID}`}
            className="navbar-brand font-weight-bold text-uppercase text-base pace-primary-color dashboard-lead companyDisplay"
          >
            {roleID === 1 ? companyProfile.companyName : fullName}
          </Link>
          <ul className="ml-auto d-flex align-items-center list-unstyled mb-0">
            ...
            <li
              ...
              h="7"
              data-tut="reactour__profileImage"
            >
              <a
                id="userInfo"
                href="."
